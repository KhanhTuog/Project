...
namespace Project
{
    public partial class CashierOderForm : UserControl
    {
        ...
        {
            using (MySqlConnection connect = new MySqlConnection(@"server=localhost;user id=root;password=;database=coffeshopproject;port=3306"))
            {
                connect.Open();
                    ...
                    , MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    if (connect.State == ConnectionState.Closed)
                    {
                        try
                        {
                            ...
                            string insertData = "INSERT INTO customers (customer_id, total_price, amount, `changes`, date) " +
                                                "VALUES(@custID, @totalprice, @amount, @changes, @date)";
                            ...
                            using (MySqlCommand cmd = new MySqlCommand(insertData, connect))
                            {
                                ...
                                cmd.Parameters.AddWithValue("@amount", cashierOrderForm_amount.Text); // Đảm bảo là số
                                cmd.Parameters.AddWithValue("@changes", cashierOrderForm_change.Text); // Đảm bảo là số
        ...
        private void CashierOderForm_Load(object sender, EventArgs e)
        {
